function [ Result_Auc,Result_Max_pre ] = Fun_Auc2( R_Wdr,WdrOrg,TestIds )
...
%   Detailed explanation goes here
    ...
    [i1,i2]=size(A_DresultMat_TPR);
    if i1==1
    DresultMat_TPR = A_DresultMat_TPR;
    DresultMat_FPR = A_DresultMat_FPR;
    DresultMat_Pre = A_DresultMat_Pre;
    else
    DresultMat_TPR = mean(A_DresultMat_TPR);
    DresultMat_FPR = mean(A_DresultMat_FPR);
    DresultMat_Pre = mean(A_DresultMat_Pre);
    end
    Result_Auc = trapz(DresultMat_FPR,DresultMat_TPR);
    if(isnan(Result_Auc))
        disp('error');
    end
    Result_Max_pre = max(DresultMat_Pre(:));
