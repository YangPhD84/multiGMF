function score_matrix = DDASKF(Input, P_TMat)

%参数赋值
beta  = Input.beta;
lamuda = Input.lamuda;
% predictSimMatdcChemical = Input.predictSimMatdcChemical;
% predictSimMatdg = Input.predictSimMatdg;
% predictSimMatdcGo = Input.predictSimMatdcGo;
% predictSimMatdcDomain = Input.predictSimMatdcDomain;
drug_ChemS = Input.drugChemS;
% drug_DDIS = Input.drugDDIS;
drug_AtcS = Input.drugAtcS;
% drug_SideS = Input.drugSideS;
drug_TargetS = Input.drugTargetS;
disease_DoS = Input.diseaseDoS;
disease_PhS = Input.diseasePhS;


K1 = [];
K1(:,:,1)=disease_DoS;
K1(:,:,2)=disease_PhS;
K_COM1=SKF({K1(:,:,1),K1(:,:,2)},15,10,0.2);
...
% K2(:,:,4)=drug_SideS;
K2(:,:,3)=drug_TargetS;
K_COM2=SKF({K2(:,:,1),K2(:,:,2),K2(:,:,3)},10,10,0.7);

%     K1 = [];
%     K1(:,:,1)=predictSimMatdg;
%     K1(:,:,2)=interaction_similarity(P_TMat,'1' );
%     K_COM1=SKF({K1(:,:,1),K1(:,:,2)},10,10,0.7);
%     
%     K2 = [];
%     K2(:,:,1)=predictSimMatdcChemical;
%     K2(:,:,2)=predictSimMatdcGo;
%     K2(:,:,3)=interaction_similarity(P_TMat,'2' );
%     K_COM2=SKF({K2(:,:,1),K2(:,:,2),K2(:,:,3)},15,10,0.2);
    
    score_matrix = LapRLS(K_COM1,K_COM2,P_TMat,lamuda,beta);
