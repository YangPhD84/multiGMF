...
% %% LRSSL test
% load LRSSL_dataset
% Wrr = (drug_Domain + drug_Go + drug_Pubchem)/3;
% Wdd = disease_similarity;
% Wdr = didr;
% Wrd = Wdr';
% tic
...
T=[Wrr,P_TMat';P_TMat,Wdd];  
[t1,t2]=size(T);
trIndex = double(T~=0); 

koko=1;
for alpha=1%[ 0.1 1 10 100]
for beta=10%[ 0.1 1 10 100]

[XX,WW,iter,stop1,stop2] = fBNNR_econ(alpha,beta,T,trIndex,tol1,tol2,maxiter,0,1);
M_ResultMat=WW((t1-dn+1):t1,1:dr);                       
...
save Fdataset_STresult_BNNR_10CV_fold_results.mat ...
