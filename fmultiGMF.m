...
%Options.lambda_soft lambda1 lambda2 lambda3 MaxIter
...
for t=1:Options.MaxIter
    ...
    for k = 1:GraphNum2
        ...
        D_aux{k} = inv(Options.mu2*Options.lambda_soft*Dweight(k)*ED + Options.lambda2*Ld{k})*(Options.mu2*Dweight(k)*H);
    
    end
    
    %Update W,H
    Sr=zeros(size(R_aux{1}));
    for k=1:GraphNum1
