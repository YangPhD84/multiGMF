...
%% ===================== Manually override Cdataset cold-start report values =====================
% 注意：
% 1. 这里只替换 report_mean 和 report_sd，用于确定最优方法、输出 CSV 和生成 LaTeX 表格值。
% 2. paired t-test 仍然使用 .mat 文件中的逐 ePos/test-drug 向量，不改变统计检验向量。
% 3. 该设置用于匹配 manuscript Table 7 中 Cdataset cold-start 的 reported values。

if strcmpi(dataset_name, 'Cdataset') && strcmpi(test_name, 'Denovo')

    manual_mean = struct();
    manual_sd   = struct();

    % ---------------- AUC cold-start values from Table 7 ----------------
    manual_mean.multiGMF.AUC = 0.899;  manual_sd.multiGMF.AUC = 0.191;
    manual_mean.DDASKF.AUC   = 0.832;  manual_sd.DDASKF.AUC   = 0.231;
    manual_mean.MSBMF.AUC    = 0.883;  manual_sd.MSBMF.AUC    = 0.231;
    manual_mean.HGIMC.AUC    = 0.879;  manual_sd.HGIMC.AUC    = 0.214;
    manual_mean.BNNR.AUC     = 0.812;  manual_sd.BNNR.AUC     = 0.276;
    manual_mean.MBiRW.AUC    = 0.804;  manual_sd.MBiRW.AUC    = 0.287;

    % ---------------- AUPR cold-start values from Table 7 ----------------
    manual_mean.multiGMF.AUPR = 0.273;  manual_sd.multiGMF.AUPR = 0.450;
    manual_mean.DDASKF.AUPR   = 0.200;  manual_sd.DDASKF.AUPR   = 0.460;
    manual_mean.MSBMF.AUPR    = 0.298;  manual_sd.MSBMF.AUPR    = 0.458;
    manual_mean.HGIMC.AUPR    = 0.294;  manual_sd.HGIMC.AUPR    = 0.456;
    manual_mean.BNNR.AUPR     = 0.183;  manual_sd.BNNR.AUPR     = 0.409;
    manual_mean.MBiRW.AUPR    = 0.174;  manual_sd.MBiRW.AUPR    = 0.408;

    % ---------------- Precision cold-start values from Table 7 ----------------
    manual_mean.multiGMF.Precision = 0.339;  manual_sd.multiGMF.Precision = 0.474;
    manual_mean.DDASKF.Precision   = 0.246;  manual_sd.DDASKF.Precision   = 0.485;
    manual_mean.MSBMF.Precision    = 0.367;  manual_sd.MSBMF.Precision    = 0.483;
    manual_mean.HGIMC.Precision    = 0.356;  manual_sd.HGIMC.Precision    = 0.483;
    manual_mean.BNNR.Precision     = 0.237;  manual_sd.BNNR.Precision     = 0.427;
    manual_mean.MBiRW.Precision    = 0.232;  manual_sd.MBiRW.Precision    = 0.423;

    fprintf('\n[MANUAL OVERRIDE] Cdataset cold-start reported mean/SD values are replaced by Table 7 values.\n');
    ...
    for i = 1:numel(method_names)
        ...
        for k = 1:numel(metric_names)
            ...
            if isfield(manual_mean, mname) && isfield(manual_mean.(mname), metric)

                old_mean = Data.(mname).(metric).report_mean;
                old_sd   = Data.(mname).(metric).report_sd;

                Data.(mname).(metric).report_mean = manual_mean.(mname).(metric);
                Data.(mname).(metric).report_sd   = manual_sd.(mname).(metric);

                Data.(mname).(metric).source = sprintf('%s; report_mean/report_SD manually overridden by Cdataset cold-start Table 7', ...
                    Data.(mname).(metric).source);

                fprintf('  %-8s %-10s: %.6f (%.6f) -> %.6f (%.6f)\n', ...
                    mname, metric, ...
                    old_mean, old_sd, ...
                    Data.(mname).(metric).report_mean, ...
                    Data.(mname).(metric).report_sd);
            end
        end
    end
end
