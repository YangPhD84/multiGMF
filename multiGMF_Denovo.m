%%======================== cold-start ========================%%
% This script performs the drug-side cold-start evaluation.
...
% Drug-side cold-start uses drug chemical, ATC, and target similarities. Side-effect and drug-drug interaction similarities are excluded because new drugs may not have reliable side-effect or DDI information.
...
% Drug-side cold-start setting.  Select drugs with only one known disease association. The known association of each selected drug is removed and used as the test association.
NumAS = sum(Wdr);
...
for num = 1:p_drugLen
    ...
    % Mask the known association of the test drug before WKNN preprocessing and model training to avoid information leakage.
...
M_ResultMat=WW';   
    ...
    % Restore the masked association before moving to the next test drug.
...
save Fataset_STresult_multiGrNMF_Denovoone ...
