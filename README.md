...
For each dataset, evaluation setting, and metric, the method with the highest reported point estimate was compared with each of the other five methods using a two-sided paired t-test. For repeated 10-fold cross-validation, this point estimate is the mean across the ten repeated runs; for drug-side cold-start testing, it is the macro-curve estimate stored in the MAT file. The five raw p-values were adjusted using the Holm procedure with `alpha = 0.05`.
